...
if ($_GET['teamId']) {
    $teamId = $_GET['teamId'];
    $sql = "SELECT * FROM teamb WHERE teamId = {$teamId}";
    $result = mysqli_query($connect, $sql);
    if (mysqli_num_rows($result) == 1) {
        $data = mysqli_fetch_assoc($result);
        $team_name = $data['teamB_name'];
        $logo = $data['logo'];
...
<?php require_once '../events/components/_navbar.php' ?>
    ...
    <fieldset class="fieldset-forms">
        ...
        <form action="actions/b_update.php" method="post" enctype="multipart/form-data">
            <table class='table'>
                <tr>
                    <th>Team name</th>
                    <td><input class='form-control' type="text" name="teamB_name" value="<?php echo $team_name ?>" /></td>
                </tr>
